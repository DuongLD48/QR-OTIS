/* 
 * Fast & Optimal QR Code generator library (JavaScript)
 * Based on Nayuki's QR Code generator (MIT License)
 * Correct, compact, supports all 40 versions, 4 ECC levels, Alphanumeric/Numeric optimization, and Boost ECC.
 */
...
(function (qrcodegen) {
    var QrCode = /** @class */ (function () {
        ...
        QrCode.encodeSegments = function (segs, ecl, minVersion, maxVersion, mask, boostEcc) {
            if (minVersion === void 0) { minVersion = 1; }
            if (maxVersion === void 0) { maxVersion = 40; }
            if (mask === void 0) { mask = -1; }
            if (boostEcc === void 0) { boostEcc = true; }
            ...
            if (bb.length % 8 != 0)
                throw new Error("Assertion error");
            for (var padByte = 236; bb.length < dataCapacityBits; padByte ^= 253)
                appendBits(padByte, 8, bb);
            var dataCodewords = [];
            while (dataCodewords.length * 8 < bb.length)
                dataCodewords.push(0);
            bb.forEach(function (b, i) {
                if (b) dataCodewords[i >>> 3] |= 1 << (7 - (i & 7));
            });
        ...
        QrCode.prototype.getAlignmentPatternPositions = function () {
            if (this.version == 1) return [];
                ...
                var result = [6];
                for (var pos = this.size - 7; result.length < num; pos -= step)
        ...
        QrCode.prototype.finderPenaltyCountPatterns = function (runHistory) {
            ...
            if (n > 0 && runHistory[2] == n && runHistory[3] == n * 3 && runHistory[4] == n && runHistory[5] == n) {
                var core = n * 7;
                return (runHistory[0] >= core && runHistory[6] >= n * 4 || runHistory[6] >= core && runHistory[0] >= n * 4) ? 1 : 0;
            }
            return 0;
        };
        ...
        QrCode.getNumRawDataModules = function (ver) {
            if (ver < QrCode.MIN_VERSION || ver > QrCode.MAX_VERSION) throw new RangeError("Version out of range");
            var size = ver * 4 + 17;
            var numAlign = Math.floor(ver / 7) + 2;
            var result = size * size - 64 * 3 - (size - 16) * 2 - (ver >= 7 ? 36 : 0) - (ver >= 2 ? (numAlign * numAlign - 3) * 25 - (numAlign - 2) * 2 * 10 : 0);
            return result;
        };
    ...
    var QrSegment = /** @class */ (function () {
        ...
        QrSegment.makeSegments = function (text) {
            ...
            else return [QrSegment.makeBytes(qrcodegen.QrSegment.toUtf8ByteArray(text))];
        };
    ...
    (function (QrCode) {
        var Ecc = /** @class */ (function () {
            function Ecc(ordinal, formatBits, name) {
                this.ordinal = ordinal;
                this.formatBits = formatBits;
                this.name = name;
            }
            Ecc.LOW = new Ecc(0, 1, "Low (~7%)");
            Ecc.MEDIUM = new Ecc(1, 0, "Medium (~15%)");
            Ecc.QUARTILE = new Ecc(2, 3, "Quartile (~25%)");
            Ecc.HIGH = new Ecc(3, 2, "High (~30%)");
            return Ecc;
        }());
        QrCode.Ecc = Ecc;
    ...
    (function (QrSegment) {
        var Mode = /** @class */ (function () {
            function Mode(modeBits, numBitsCharCount, name) {
            ...
            Mode.NUMERIC = new Mode(1, [10, 12, 14], "numeric");
            Mode.ALPHANUMERIC = new Mode(2, [9, 11, 13], "alphanumeric");
            Mode.BYTE = new Mode(4, [8, 16, 16], "byte");
            Mode.KANJI = new Mode(8, [8, 10, 12], "kanji");
            Mode.ECI = new Mode(7, [0, 0, 0], "eci");
            return Mode;
        }());
        QrSegment.Mode = Mode;
